...
控制类请求（上一首、下一首、跳转、暂停、继续、停止）只操作当前队列，不重新读取凭证。歌单、收藏、每日推荐和私人 FM 等登录专属操作在匿名状态下会明确提示“让当前设备扫码登录”。普通歌曲搜索仍可匿名播放 `fee=0` 或 `fee=8` 的公开免费歌曲。
...
- 音源仍必须属于网易云域名，不调用解灰或替换音源。缓存容量、TTL、单文件 40 MB 限制及播放队列保护沿用原播放插件实现。
