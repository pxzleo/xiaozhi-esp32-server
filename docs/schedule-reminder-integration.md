...
请求不得有额外字段；`source_id` 是正整数，`label` 去空白后为 1–80 个 Unicode 字符，`topic=follow_up`、`priority=normal`、`reason="schedule follow up"`、`follow_up=true`、`requires_response=true`、`speak=true`。服务端不调用 LLM，固定播报“刚才提醒的 `{label}` 完成了吗？”，并沿用现有 TTS 和自动收听链。`expires_at` 不晚于服务端当前时间时直接拒绝。同连接重复 `event_id` 使用容量 256 的 FIFO 集合抑制；跨连接在播报前检查 manager 幂等事件，已投递事件不得重播。65–96 字符的设备事件 ID 会确定性映射为不超过 64 字符的 manager 审计 ID。无效通知日志不包含 `label`。
...
同一 follow-up 的 outcome 回写必须等待该事件的审计创建与初始投递状态完成；只有实际音频完成才写 `delivered`，断连、打断、替换或音频失败均写 `failed`。二者共享单一异步任务顺序，保证不会在事件创建前更新失败，也不会让较晚的 `outcome=none` 覆盖已确认的终态。设备工具成功结果还必须返回与当前事件一致的 `data.source_id`，不匹配时不得回写 outcome。
