...
手机 M3 的 `pending/claim.expires_at` 固定为 Unix 毫秒整数；`claim_token` 固定为小写 UUID v4，REST DTO、手机 WS `external_context` 帧和 Android 使用同一 ASCII 正则。complete 及 claimed-context 查询使用数据库 `CURRENT_TIMESTAMP(3)` 同时校验 180 秒领取租约和事件有效期，旧 token 不能写终态或读取上下文；事件行与 owner-scoped delivery ledger 在同一事务同步，任一 CAS 失败均回滚。新闻上下文以 `system` 角色的低信任结构化资料加入会话，明确禁止模型遵循或解释标题、事实字段中的指令、角色变更和工具调用要求，绝不作为 assistant 历史消息。
